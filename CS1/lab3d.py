...
def lsystemToDrawingCommands(drawRules, lstr):
    ...
    for c in lstr:
        if c == '[':
            locs.append([x, y, face])
        elif c == ']':
            laLoc = locs.pop()
            cmd = 'G {0} {1} {2}'.format(laLoc[0], laLoc[1], laLoc[2])
            total.append(cmd)
            tup = nextLocation(x, y, face, cmd)
            x = tup[0]
            y = tup[1]
            face = tup[2]
        elif c in drawRules:
            total.append(drawRules[c])
            tup = nextLocation(x, y, face, drawRules[c])
            x = tup[0]
            y = tup[1]
            face = tup[2]
    return total
